...
namespace tsp {
	...
	__device__ __host__
	inline int distance(GeolocationKM metric, float lat1, float lng1, float lat2, float lng2) {
		float lat1_rad = M_PI * lat1 / 180.0f;
		float lat2_rad = M_PI * lat2 / 180.0f;
		float lng_diff_rad = M_PI * (lng1 - lng2) / 180.0f;
		...
		return (int)(6378.388f * atan2f(sqrtf(q1*q1 + q2*q2), q5) + 1.0f);
	}
}
