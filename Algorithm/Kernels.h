...
namespace tsp {
	...
	__global__ void geneticOXAlgorithmKernel(const Instance instance, int* fitness, int* population, curandState* globalState, float crossoverProbability, float mutationProbability, int maxIterations, int* results) {
		...
		int* result = results + (tid * instanceSize);
	...
	template <typename Instance>
	__global__ void tspPMXAlgorithmKernel(const Instance instance, int* fitness, int* population, int* bestFitness, curandState* globalState,
		...
		for (int iteration = 0; iteration < maxIterations; ++iteration) {
			...
			if (!(elitism && (tid == sharedFitnessIndex[0] || tid + 1 == sharedFitnessIndex[0])) && curand_uniform(&localState) < crossoverProbability) {
				PMX(chromosome[0], chromosome[1], result[0], result[1], instanceSize, &localState);
			}
			__syncthreads();
			// Mutation
			for (int i = 0; i < 2; ++i) {
	...
	__global__ void tspRandomStepAlgorithmKernel(const Instance instance, int* fitness, int* population, int* bestFitness, curandState* globalState,
		int maxIterations, int maxIterationStop, float crossoverProbability, float mutationProbability, bool elitism, int* results)
	{
		__shared__ int totalFitness[128];
		__shared__ int sharedFitness[128];
		__shared__ int sharedFitnessIndex[128];
		...
		int* chromosome[2] = { population + tid * instanceSize, population + (tid + 1) * instanceSize };
		int* result[2];
		result[0] = results + (tid * 2 * instanceSize);
		result[1] = results + ((tid * 2 + 1) * instanceSize);
		bool flags[] = { false, false };
